...
public class RSAModExpFullyVariable2 extends CircuitGenerator {
  ...
  public static void main(String[] args) {
    ...
    new RSAModExpFullyVariable2();
  }

  public RSAModExpFullyVariable2() {
    super("RSAModExpFullyVariable2");
    __generateCircuit();
    this.__evaluateSampleRun(new SampleRun("Sample_Run1", true) {
      public void pre() {
        ...
        mod.mapValue(new BigInteger(""), CircuitGenerator.__getActiveCircuitGenerator().__getCircuitEvaluator());
  ...
  public void __init() {
    a = new UnsignedInteger(4096, new BigInteger("0"));
    exponent = new UnsignedInteger(4096, new BigInteger("0"));
    mod = new UnsignedInteger(4096, new BigInteger("0"));
    b = new UnsignedInteger(4096, new BigInteger("0"));
  }
  ...
  public void __defineInputs() {
    ...
    a = UnsignedInteger.createInput(this, 4096);
    exponent = UnsignedInteger.createInput(this, 17);
    mod = UnsignedInteger.createInput(this, 4096);
  ...
  public void outsource() {
    b.assign(new UnsignedInteger(4096, new BigInteger("1")), 4096);
    Bit[] expBits = exponent.getBitElements();
    for (int i = expBits.length - 1; i >= 0; i--) {
      b.assign(b.mul(b).mod(mod), 4096);
      {
        Bit bit_b0c0v_0 = expBits[i].copy();
        boolean c_b0c0v_0 = CircuitGenerator.__getActiveCircuitGenerator().__checkConstantState(bit_b0c0v_0);
        if (c_b0c0v_0) {
          if (bit_b0c0v_0.getConstantValue()) {
            b.assign(b.mul(a).mod(mod), 4096);
          } else {

          }
        } else {
          ConditionalScopeTracker.pushMain();
          ConditionalScopeTracker.push(bit_b0c0v_0);
          b.assign(b.mul(a).mod(mod), 4096);
